...
import { DriverOnlineStatus, DriverAvailabilityStatus, TripStatus } from '../src/constants/tripStatus';
...
describe('2GO Full Ride-Hailing Vertical Slice Lifecycle E2E', () => {
  it('should successfully execute entire ride lifecycle: Auth -> Online -> Estimate -> Dispatch -> Arrive -> Start -> Stream -> Complete -> Pay -> Rate -> Ops Dashboard', async () => {
    ...
    expect(locRes.body.data.currentLocation.coordinates).toEqual([-73.9855, 40.7485]);

    // 3. Customer checks fare estimate
    const estRes = await request(app)
      .post('/api/v1/pricing/estimate')
      .send({
        pickup: { coordinates: [-73.9851, 40.7488], address: 'Empire State Building' },
        destination: { coordinates: [-73.9818, 40.7533], address: 'Grand Central Terminal' },
        category: VehicleCategory.STANDARD,
      });
